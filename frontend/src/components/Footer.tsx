...
const socials = [
  ...
  { icon: Mail, href: "tompevaishnavi89@gmail.com", label: "Email" },
];
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
        <div className="flex flex-col md:flex-row items-center justify-between gap-8">
          ...
          <div className="flex flex-col items-center md:items-start gap-3">
            <a href="#home" className="flex items-center gap-2">
              ...
              <span className="font-bold text-base" style={{ fontFamily: "var(--font-space-grotesk)" }}>
                <span className="gradient-text">Dev</span>
                <span className="text-slate-200">Portfolio</span>
              </span>
            </a>
            <p className="text-slate-500 text-sm text-center md:text-left max-w-xs">
              GenAI & Full Stack Developer building intelligent, scalable web experiences.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            {socials.map(({ icon: Icon, href, label }) => (
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="w-9 h-9 rounded-lg bg-slate-800/60 border border-slate-700/50 flex items-center justify-center text-slate-400 hover:text-blue-400 hover:border-blue-500/40 hover:bg-blue-500/10 transition-all"
                aria-label={label}
              >
                <Icon />
              </motion.a>
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/40 text-center">
          <p className="text-slate-600 text-sm flex items-center justify-center gap-1">
            ...
            by{" "}
            <span className="text-slate-400 font-medium">Vaishnavi</span> <Heart size={13} className="text-red-500" fill="currentColor" /> · © {new Date().getFullYear()}
          </p>
        </div>
      </div>
    </footer>
